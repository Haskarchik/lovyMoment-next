/**
 * EN category landing page (`/en/atractions`, `/en/animators`, …).
 * Mirrors the UK route at `app/[category]/page.tsx` with locale='en'.
 */
...
export default async function CategoryPage({ params }: Params) {
  ...
  const itemListLd = {
    ...
    itemListElement: products.map((p, idx) => ({
      '@type': 'ListItem',
      position: idx + 1,
      url: `${SITE_URL}/en/${category}/${p.slug}`,
      name: p.name
    }))
  };

  const breadLd = breadcrumbLd([
    { name: 'Lovy Moment', url: `${SITE_URL}/en` },
    { name: heading, url: `${SITE_URL}/en/${category}` }
  ]);
